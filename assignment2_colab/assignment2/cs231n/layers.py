...
def relu_backward(dout, cache):
    ...
    ind = np.zeros_like(x)
    ind[np.nonzero(np.maximum(0, x))] = 1
    dx = np.multiply(ind, dout)
...
def softmax_loss(x, y):
    ...
    loss_i = np.log(np.sum(np.exp(scores), axis=1)) - scores[np.arange(num_train), y]
    loss = np.sum(loss_i)

    loss /= num_train

    dx = np.exp(scores) / np.sum(np.exp(scores), axis=1)[:, np.newaxis]
...
def batchnorm_backward_alt(dout, cache):
    ...
    dx_norm = dout * gamma
    dx = 1/N / std * (N * dx_norm - dx_norm.sum(axis=0) - x_norm * (dx_norm * x_norm).sum(axis=0))
...
def layernorm_forward(x, gamma, beta, ln_param):
    ...
    std = np.sqrt(sample_var + eps)
    x_norm = (x - sample_mean[:, np.newaxis]) / std[:, np.newaxis]
    out = gamma * x_norm + beta

    cache = (x_norm, std, gamma)
...
def layernorm_backward(dout, cache):
    ...
    N = dout.shape[0]
    x_norm, std, gamma = cache

    dgamma = (dout * x_norm).sum(axis=0)
    dbeta = dout.sum(axis = 0)

    dx_norm = dout * gamma
    dx = 1/N / std[:, np.newaxis] * (N * dx_norm - dx_norm.sum(axis=1)[:, np.newaxis] - x_norm * (dx_norm * x_norm).sum(axis=1)[:, np.newaxis])
...
def conv_forward_naive(x, w, b, conv_param):
    ...
    for n in range(N):
      for f in range(F):
        for i in range(H_):
          for j in range(W_):
            out[n, f, i, j] += np.sum(x_pad[n, :, stride*i:stride*i + HH, stride*j:stride*j + WW] * w[f, :, :, :]) + b[f]
...
def conv_backward_naive(dout, cache):
    ...
    #           for l in range(WW):
    #             for c in range(C):
    #               dw[f, c, k, l] += x_pad[n, c, stride*i+k, stride*j+l] * dout[n, f, i, j]
    ...
    #           for f in range(F):
    #             for c in range(C):
    #               dx_pad[n, c, stride*i+k, stride*j+l] += dout[n, f, i, j] * w[f, c, k, l]
    
    for n in range(N):
      for i in range(H_):
        for j in range(W_):
          for f in range(F):
            dx_pad[n, :, stride*i:stride*i + HH, stride*j: stride*j + WW] += dout[n, f, i, j] * w[f, :, :, :]
...
def max_pool_forward_naive(x, pool_param):
    ...
    for i in range(H_):
      for j in range(W_):
        out[:, :, i, j] = np.max(x[:, :, stride*i:stride*i+pool_height, stride*j:stride*j+pool_width], axis=(2, 3))
